...
struct ShortContentDeposit : public ShortContentDepositBasic,
                             public xchain::Contract {
public:
    void initialize() {
    ...
    void storeShortContent() {
        ...
        const std::string userKey =
            std::string(UserBucket) + "/" + user_id + "/" + topic + "/" + title;
        if (topic.length() > TOPIC_LENGTH_LIMIT ||
            title.length() > TITLE_LENGTH_LIMIT ||
            content.length() > CONTENT_LENGTH_LIMIT) {
            ctx->error(
                "The length of topic or title or content is more than "
                "limitation");
            return;
        }
    ...
    void queryByUser() {
        xchain::Context* ctx = this->context();
        std::string key =
            std::string(UserBucket) + "/" + ctx->arg("user_id") + "/";
    ...
    void queryByTopic() {
        ...
        std::string topic = ctx->arg("topic");
        std::string key =
            std::string(UserBucket) + "/" + user_id + "/" + topic + "/";
        std::string result;
        std::unique_ptr<xchain::Iterator> iter =
            ctx->new_iterator(key, key + "~");
        while (iter->next()) {
...
DEFINE_METHOD(ShortContentDeposit, initialize) { self.initialize(); }

DEFINE_METHOD(ShortContentDeposit, storeShortContent) {
    self.storeShortContent();
}

DEFINE_METHOD(ShortContentDeposit, queryByUser) { self.queryByUser(); }

DEFINE_METHOD(ShortContentDeposit, queryByTitle) { self.queryByTitle(); }

DEFINE_METHOD(ShortContentDeposit, queryByTopic) { self.queryByTopic(); }
